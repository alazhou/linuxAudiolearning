...
static int soc_bind_dai_link(struct snd_soc_card *card, int num)
{
	...
	dev_err(card->dev, "ASoC: CODEC %s binding fuck!!\n",dai_link->codec_name);
	...
	dev_err(card->dev, "ASoC: CPU DAI %s find !!!!\n",
	...
	list_for_each_entry(codec, &codec_list, list) {
		...
		dev_err(card->dev, "ASoC: CODEC DAI %s find !!!!\n",
				dai_link->codec_dai_name);
